...
import common
import metrics
from sn_utils import plot_segmentation_images
"""

from . import common_sn
from . import metrics_sn
from .sn_utils import plot_segmentation_images
...
class SimpleNet(torch.nn.Module):
    ...
    def load(
        ...
        """
        # modified place, because we want simplenet to connect to dictionary format of MobileNetV3 and not try to hook into a backbone internally
        feature_aggregator = common_sn.DictFeatureAggregator(
            self.backbone, self.layers_to_extract_from, self.device, train_backbone
        )
        feature_dimensions = feature_aggregator.feature_dimensions(input_shape)
        self.forward_modules["feature_aggregator"] = feature_aggregator

        preprocessing = common_sn.Preprocessing(
            feature_dimensions, pretrain_embed_dimension
        )
        self.forward_modules["preprocessing"] = preprocessing

        self.target_embed_dimension = target_embed_dimension
        preadapt_aggregator = common_sn.Aggregator(
        ...
        self.anomaly_segmentor = common_sn.RescaleSegmentor(
    ...
    def test(self, training_data, test_data, save_segmentation_images):
        ...
        auroc = metrics_sn.compute_imagewise_retrieval_metrics(
            scores, anomaly_labels
        )["auroc"]

        # Compute PRO score & PW Auroc for all images
        pixel_scores = metrics_sn.compute_pixelwise_retrieval_metrics(
    ...
    def _evaluate(self, test_data, scores, segmentations, features, labels_gt, masks_gt):
        ...
        if len(masks_gt) > 0:
            ...
            pro = metrics_sn.compute_pro(np.squeeze(np.array(masks_gt)),
                                         norm_segmentations)
        else:
            full_pixel_auroc = -1
            pro = -1
    ...
    def _predict_dataloader(self, dataloader, prefix):
        ...
        with tqdm.tqdm(dataloader, desc="Inferring...", leave=False) as data_iterator:
            for data in data_iterator:
                ...
                #    scores.append(score)
                #    masks.append(mask)
                _scores, _masks, _ = self._predict(image)
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            ...
            masks, _ = self.anomaly_segmentor.convert_to_segmentation(patch_scores, features)

...
class GLASS(torch.nn.Module):
    ...
    def trainer(self, training_data, val_data, name):
        ...
        for i_epoch in pbar:
            ...
            tqdm.tqdm.write(pbar_str)  # prints a new line and doesn't get overwritten - added later
            ...
            if (i_epoch + 1) % self.eval_epochs == 0:
                ...
                eval_path = os.path.join(self.model_dir, "results", "eval", name)
                train_path = os.path.join(self.model_dir, "results", "training", name)
    ...
    def tester(self, test_data, name):
        ...
        # NEW: fallback to last checkpoint if no best checkpoint exists
        if len(ckpt_path) == 0:
            last_ckpt = os.path.join(self.ckpt_dir, "ckpt.pth")
            if os.path.exists(last_ckpt):
                ckpt_path = [last_ckpt]
                epoch = self.meta_epochs - 1
            else:
        ...
        # keep original epoch parsing only if it was ckpt_best_XX.pth
        if "ckpt_best" in os.path.basename(ckpt_path[0]):
    ...
    def _evaluate(self, images, scores, segmentations, labels_gt, masks_gt, name, path='training'):
        ...
        for i in range(len(defects)):
            ...
            full_path = os.path.join(self.model_dir, "results", path, name)
